...
def add_pets_from_json(filename, cur, conn):
    ...
    for i in range(len(json_data)):
        cur.execute("INSERT INTO Patients (pet_id, name, species_id, age, cuteness, aggressiveness) VALUES (?,?,?,?,?,?)",(i,species[i]))
    conn.commit()



# TASK 3, use a select statement
# CODE TO OUTPUT NON-AGGRESSIVE PETS
def non_aggressive_pets(aggressiveness, cur, conn):
    cur.execute("SELECT name, aggressiveness FROM Patients WHERE agressiveness < 10")
